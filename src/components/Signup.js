...
function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post("https://echovote-backend-12678945690.us-central1.run.app/user/signup", formData);
  ...
  const handleVerify = async (e) => {
    ...
    setVerifying(true);
    try {
      const response = await axios.post("https://echovote-backend-12678945690.us-central1.run.app/user/verify", {
        email: emailForOTP,
        otp
      });
