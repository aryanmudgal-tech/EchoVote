...
function PostCard({ post, onLikeUpdate }) {
  ...
  const fetchReplies = async () => {
    ...
    try {
      const res = await axios.get(`https://echovote-backend-12678945690.us-central1.run.app/posts/${post.id}/replies`);
      setReplies(res.data);
    } catch (error) {
      setReplies([]);
    } finally {
  ...
  const handleReplySubmit = async (e) => {
    ...
    try {
      await axios.post(
        `https://echovote-backend-12678945690.us-central1.run.app/posts/${post.id}/replies`,
  ...
  const handleLike = async () => {
    ...
    try {
      const response = await axios.post(
        `https://echovote-backend-12678945690.us-central1.run.app/posts/${post.id}/like`,
        {},
        { headers: { Authorization: `Bearer ${authUser.token}` } }
      );
      if (onLikeUpdate) {
